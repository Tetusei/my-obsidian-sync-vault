...
function fetchAndFilterMail() {
  ...
  try {
    ...
    for (let i = 0; i < threads.length; i++) {
      ...
      // メモ列（M列）にセットするテキスト（添付ファイルリンク、無ければ送信元アドレスとオリジナル件名）
      const memoContent = attachmentText 
        ? `${attachmentText}\n\n送信元: ${from}\n【元件名】: ${subject}` 
        : `送信元: ${from}\n【元件名】: ${subject}`;
...
function isDuplicateEmail(sheet, subject, body, dateStr) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const existingMemo = String(data[i][Config.TODO_COL.MEMO] || "");
    
    // メモ欄から「【元件名】: 」があればそれを抽出し、無ければタイトルをフォールバックとして使用
    let cleanTitle = "";
    const originalSubjectMatch = existingMemo.match(/【元件名】:\s*(.*)/);
    if (originalSubjectMatch) {
      cleanTitle = originalSubjectMatch[1].trim();
    } else {
