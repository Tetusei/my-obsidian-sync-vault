...
function cleanUpPastDuplicateTasks() {
  ...
  // メモ欄の【元件名】優先でタイトルをクリーンにするヘルパー
  const getCleanTitle = (rowVal, memoVal) => {
    const originalSubjectMatch = String(memoVal || "").match(/【元件名】:\s*([^\n\r]*)/);
    if (originalSubjectMatch) return originalSubjectMatch[1].trim();
    let clean = rowVal ? rowVal.toString().trim() : "";
    const linkMatch = clean.match(/=HYPERLINK\("[^"]*",\s*"([^"]*)"\)/i);
    if (linkMatch) {
      clean = linkMatch[1];
    }
  ...
  for (let i = lastRow - 1; i >= 1; i--) {
    ...
    const cleanRowTitle = getCleanTitle(rowTitle, rowMemo);
    if (!cleanRowTitle) continue;
    const normRowTitle = normalizeSubject(cleanRowTitle);
    ...
    for (let j = 1; j < data.length; j++) {
      ...
      const compareMemo = data[j][Config.TODO_COL.MEMO] ? data[j][Config.TODO_COL.MEMO].toString() : "";
      
      const cleanCompareTitle = getCleanTitle(compareTitle, compareMemo);
      const normCompareTitle = normalizeSubject(cleanCompareTitle);
      
      if (normRowTitle === normCompareTitle && normalizeDate(rowDate) === normalizeDate(compareDate)) {
        isDuplicate = true;
        matchIndex = j;
        break;
      }
...
function showDuplicateTasksDialog() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const rawMemo = String(data[i][Config.TODO_COL.MEMO] || "");
    const memo = rawMemo.substring(0, 100);
    const mailLink = String(data[i][Config.TODO_COL.MAIL_LINK] || "").trim();
    
    if (!titleFormula) continue;
    
    // メモ欄の【元件名】優先でタイトルをクリーンにする
    let title = "";
    const originalSubjectMatch = rawMemo.match(/【元件名】:\s*([^\n\r]*)/);
    if (originalSubjectMatch) {
      title = originalSubjectMatch[1].trim();
    } else {
      title = titleFormula;
