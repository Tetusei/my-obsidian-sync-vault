...
function admin_setupMailTrigger() {
  ...
  ScriptApp.getProjectTriggers().forEach(t => {
    if (t.getHandlerFunction() === 'fetchAndFilterMail') { ScriptApp.deleteTrigger(t); count++; }
  });
  ScriptApp.newTrigger('fetchAndFilterMail').timeBased().everyMinutes(5).create();
  SpreadsheetApp.getActive().toast('メールの自動解析を開始しました。', '📧設定完了', 3000);
}
...
function admin_deleteMailTrigger() {
  ...
  if (count > 0) SpreadsheetApp.getActive().toast('メールの自動解析を停止しました。', '📴設定解除', 3000);
}
