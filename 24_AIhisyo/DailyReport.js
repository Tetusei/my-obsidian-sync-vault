...
function sendDailyMorningDigest() {
  ...
  // 各列のインデックスを見つけるヘルパー関数（表記揺れや空白に対応、大文字小文字無視、完全一致）
  ...
  const colState = findHeaderIndex(['ステータス', '状態', '状況', '進捗', '進捗状況', '状態・状況'], Config.TODO_COL.STATUS);
  const colLimit = findHeaderIndex(['期限', '期日', '期限日', '提出期限', '提出期日'], Config.TODO_COL.DUE_DATE);
  const colTitle = findHeaderIndex(['件名', 'タイトル', 'ToDo', 'todo', 'TODO'], Config.TODO_COL.TITLE);
  const colPic = findHeaderIndex(['担当', '担当者'], Config.TODO_COL.PIC);
  const colPriority = findHeaderIndex(['重要度', '優先', '優先度'], Config.TODO_COL.PRIORITY);
