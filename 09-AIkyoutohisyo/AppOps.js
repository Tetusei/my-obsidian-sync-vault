...
function turnOnVoiceMemoTrigger() {
  ...
  if (status === 'ALREADY_ON') {
    SpreadsheetApp.getUi().alert("ℹ️ 音声メモ連携はすでに【ON】になっています。\n\nスマホのフォームから送信すると、自動でToDoシートへ追加されます。");
  } else if (status === 'RECREATED') {
    SpreadsheetApp.getUi().alert("✅ 重複していた音声メモ連携トリガーを整理し、再度【ON】にしました！\n\nスマホのフォームから送信すると、自動でToDoシートへ追加されます。");
  } else {
...
function turnOffVoiceMemoTrigger() {
  ...
  if (deleted) {
    SpreadsheetApp.getUi().alert("❌ 音声メモ連携を【OFF】にしました。\n\nフォームからの自動取り込みを停止しました。");
  } else {
    SpreadsheetApp.getUi().alert("ℹ️ 音声メモ連携はすでに【OFF】（未設定）になっています。");
  }
}
