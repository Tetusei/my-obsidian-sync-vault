...
| `test_reserve.js` | 予備コマ。2つの入り口、作り直しで消えないこと、手入力を取り込まないこと、色 | 71 |
| `test_ng.js` | だめなコマの指定・読み取り・枠への反映・再生成での引き継ぎ | 73 |
| `test_warn.js` | 予約とだめなコマがぶつかったときの警告（5つの画面すべて） | 62 |
| `test_guard.js` | 作り直される場所の保護、手入力の警告と記録、データ点検 | 57 |
| `test_views.js` | 予約と表示更新の分離、自動更新のタイマー、設定シートからの切替 | 45 |
| `test_reset.js` | 3段階のリセット。何が消えて何が残るか。配布用の初期化 | 86 |

**合計 643項目。** 数のうえでは元の約280項目を超えたが、中身は同じではない。
v4.6.13〜17 の変更に合わせて書き直しているので、残りも同じようには戻らない。
いま守れているのは「予約が消える／二重に入る」系の中核、だめなコマ、手入力の防止、
表示更新の分離。残りは、クラスの増減・自動処理・点検・並び順・QR。
...
- 罫線・フォント・列幅。鎖でつなげるだけの空実装
  （**背景色だけは本当に覚えている。** 空実装にすると「予約済と未予約が同じ色」でも
  テストが通ってしまう。v4.6.16 の不具合がまさにそれだった）
- 実行時間の上限、同時実行
