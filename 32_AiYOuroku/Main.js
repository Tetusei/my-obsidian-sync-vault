...
function generateFromDailyRecordsInternal(allRows) {
  ...
  try {
    ...
    for (let r = startRow; r <= lastRow; r++) {
      ...
      let fixedInfoText = "";
      if (committee || club || specialNote) {
        fixedInfoText = `\n【生徒の基本情報（これも特徴として考慮に含めてください）】\n`;
...
function generateAICommentsInternal(allRows) {
  ...
  if (!isReportCard && !isYouroku) return;
  ...
  // セキュリティチェック（個人名の混入防止）
  const warningRows = scanForPersonalInfo(sheet, startRow, lastRow, isReportCard, isYouroku, CONFIG.COL_FEATURES_START, CONFIG.COL_FEATURES_END);
...
【対象者の特徴】\n${features.join('、 ')}\n出力は所見の文章のみとしてください。`;
      ...
      // ----------------------------------------------------
        ...
        // 学校種別に応じて要録のプロンプト（3段落構成・過去形指定など）を切り替え
